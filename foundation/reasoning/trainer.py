"""Reasoning-chain trainer: pretrained weights -> chain-of-thought checkpoint (M13).

Loads a pretrained (M10) or SFT-aligned (M11) or DPO-aligned (M12) checkpoint,
trains with response-masked cross-entropy on a packing dataset of structured
reasoning chains (Problem → numbered Reasoning steps → Final Answer), and
writes best/final checkpoints plus a provenance report under the same M01
reproducibility rules as pretraining and SFT.
"""
...
def train_reasoning(cfg: ReasoningConfig, log: Callable[[str], None] = print) -> dict:
    ...
    if cfg.block_size > model_cfg.block_size:
        raise ValueError(
            f"reasoning block_size {cfg.block_size} exceeds model block_size {model_cfg.block_size}"
        )
    ...
    examples = load_reasoning_examples(cfg.dataset_path)
    data_train = ReasoningDataset(
        examples,
        tokenizer,
        cfg.block_size,
        seed=cfg.seed,
    ...
    if data_train.n_blocks == 0:
        raise ValueError("reasoning train split is empty")
    if data_eval.n_blocks == 0:
        raise ValueError("reasoning eval split is empty")
    ...
    for step in range(1, cfg.max_steps + 1):
        ...
        if cfg.eval_steps and (step % cfg.eval_steps == 0 or step == cfg.max_steps):
            eval_loss, ppl, n_supervised = _evaluate(
                model, data_eval, cfg.eval_examples, device
            )
            log(
                f"step {step} eval_loss {eval_loss:.4f} ppl {ppl:.2f} (n={n_supervised})"
            )
            if eval_loss < best_eval:
                best_eval = eval_loss
                best_path = persist(step, eval_loss=eval_loss, filename=BEST_FILENAME)
                log(f"step {step} new best eval_loss {eval_loss:.4f} -> {best_path!s}")
    ...
    report = {
        ...
        "reasoning_config_digest": cfg.digest(),
        "reasoning_config": cfg.to_dict(),
        ...
        "throughput_tokens_per_sec": round(tokens_per_step * cfg.max_steps / elapsed, 1)
        if elapsed > 0
        else 0.0,
    ...
    report_path = Path(cfg.checkpoint_dir) / "reasoning_report.json"
